    ...
    const fetchData = async () => {
      try {
        const categoriesResponse = await axios.get(
          "https://blog.abhicares.com/wp-json/wp/v2/categories"
        );
        setCategories(categoriesResponse.data);

        const postsPromises = categoriesResponse.data.map(async (category) => {
          const postsResponse = await axios.get(
            `https://blog.abhicares.com/wp-json/wp/v2/posts?categories=${category.id}`
          );

          console.log("res", postsResponse);
          const postsWithMediaPromises = postsResponse.data.map(
            async (post) => {
              if (post.featured_media) {
                const featuredMediaResponse = await axios.get(
                  `https://blog.abhicares.com/wp-json/wp/v2/media/${post.featured_media}`
                );
                return {
                  ...post,
                  featured_media_data: featuredMediaResponse.data,
                };
              }
              return post;
            }
  ...
  const getSeoForAboutPage = async () => {
    try {
      const { data } = await axios.get(
        `${
          import.meta.env.VITE_APP_CMS_URL
        }/get-seo-by-page-user-side?page=blogs`
      );
                ...
                <div className={classes["category-wrapper"]}>
                          ...
                          <div className={classes["blog-thumbnail"]}>
                            {post.featured_media_data && (
                              <Link to={`/single-blog/${post.id}`}>
                                <img
                                  src={post.featured_media_data.source_url}
                                  alt=""
                                />
                              </Link>
                            )}
                  ...
                  <div className={classes["view-more-button"]}>
                    <button className={classes.btn}>
                      <Link to={`/blog-category/${categoryId}`}>View More</Link>
                    </button>
                  </div>
                </div>
              </div>
