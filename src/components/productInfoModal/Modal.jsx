...
const Modal = ({ isOpen, handleOnclick, Data, isProduct, features = [] }) => {
  const {
    res: getProductReviewRes,
    fetchData: getProductReview,
  ...
  useEffect(() => {
    const reviewObj = allReviews.find(
  ...
  const getAllProducts = async (lat, lng) => {
    getProducts(`/packages/get-package-product/${Data._id}`, {
      params: {
        latitude: lat,
        longitude: lng,
      },
  ...
  useEffect(() => {
    if (location?.geometry?.lat && location?.geometry?.lng && !isProduct) {
      getAllProducts(location?.geometry?.lat, location?.geometry?.lng);
    }
  }, [isProduct, Data._id, location]);

  useEffect(() => {
    if (getProductRes?.status === 200 || getProductRes?.status === 201) {
      console.log("getProductRes", getProductRes);

      setAllProducts(getProductRes?.data.data);
    }
  }, [getProductRes]);

  const getAllReviews = async () => {
    getProductReview(
      `/shopping/get-product-review/${Data._id}?type=${isProduct ? "product" : "package"}`,
    );
  };
  ...
  useEffect(() => {
    if (
      getProductReviewRes?.status === 200 ||
      getProductReviewRes?.status === 201
    ) {
        ...
        <div className={classes.modal_wrapper_overflow}>
          <div className={classes.modal}>
            ...
            <div className={classes.modal_body}>
              <div className={classes.border_bottom}>
                <div className={classes.modal_header}>
                  <div className={classes.modal_header_left}>
                    <h3 className={classes.modal_header_left_h4}>
                      {Data.name}
                    </h3>
                    <div className={classes.rating}>
                      <BsStarFill color="rgb(255, 138, 0)" size={20} />
                      <span className={classes.rating_span}>
              ...
              {!isProduct && !isProductLoading && allProducts.length === 0 && (
              ...
              <div className={classes.customer_reviews_section}>
                ...
                <div className={classes.customer_reviews_container}>
                  ...
                  {!isReviewLoading && allReviews.length === 0 && (
                    <p>No reviews found</p>
                  )}

                  {isReviewLoading && allReviews.length === 0 && <Loader />}
                ...
                <div className={classes.features_container}>
                  <h4>We Offer</h4>
                  {features?.length === 0 && <p>No features found</p>}
