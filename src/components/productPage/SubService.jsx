...
const SubService = ({
  ...
  useEffect(() => {
    const filtered = cart?.items?.find(
      (item) => item?.packageId?._id === singlePackage._id,
    );
  ...
  const handleOnMinusClick = async () => {
    await dispatch(
      deleteItemFromCart({
        itemId: productInCart.packageId._id,
        type: "package",
      }),
    );
    await dispatch(
  ...
  return (
    <>
      <div className={classes.sub_service}>
          ...
          <div className={classes.info_container_left}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
              <ReactStars
                ...
                color2={"#FF8A00"}
              />
              {singlePackage.rating}({singlePackage?.totalReviews})
            </div>
        ...
        <button onClick={handleOnclick} className={classes.package_button}>
          View details
        </button>
      </div>
      {isOpen && (
